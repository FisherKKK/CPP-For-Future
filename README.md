...
| `eof`             | 流的`eof`置位返回`true`                      |
| `fail`            | 流的`fail`或者`bad`置位                      |
| `bad`             | 流的`bad`置位                                |
| `good`            | 流处于有效状态                               |
| `clear`           | 将流的所有状态复位                           |
| `clear(flags)`    | 将流给定的flag置位                           |
| `setstate(flags)` | 设置对应的状态                               |
| `rdstate()`       | 返回当前条件状态                             |
...
`IO`库定义了一个平台无关的`iostate`类型, 提供了表达流状态的完整功能. 

`badbit`表示系统级错误, 如不可恢复的读写错误, 通常情况下一旦`badbit`被置位, 流就无法再使用. 在发生可恢复错误之后, `failbit`被置位, 如期望读取数值却读出一个字符等错误, 这种问题可以修复, 流还可以继续使用. 如果达到文件结束位置, `eofbit`和`failbit`都会置位. `goodbit`的值为0, 表示流未发生错误.



##### 管理条件状态

流对象`rdstate`成员返回一个`iostate`值, 对应流的当前状态. `setstate`操作将给定条件位置位, 表示发生了对应的错误. `clear`成员是一个重载的成员, 它有一个不接受参数的版本, 另一个版本接受一个`iostate`类型的参数
...
文本串可能立即打印出来, 也可能保存缓冲区, 随后在打印. 有了缓冲机制, 操作系统可以将程序的多个输出操作组合成单一的系统级写操作, 由于系统的写操作可能很耗时, 允许操作系统将多个输出操作组合位单一的设备写操作可以带来很大的性能提升.

导致缓冲刷新的操作原因很多:

* 程序正常结束, 作为`main`函数的`return`操作的一部分, 缓冲刷新被执行
* 缓冲区满时, 需要刷新缓冲, 而后新的数据才能继续写入缓冲区
* 我们可以采用`endl`来显式刷新缓冲区
* 在每个输入操作之后, 我们可以使用操纵符`unitbuf`设置流的内部状态, 来清空缓冲区. 默认情况下, 对`cerr`是设置`unitbuf`的, 因此写入`cerr`的内容都是立即刷新
* 一个输出流可能被关联到另一个流, 当读写被关联的流时, 关联到的流的缓冲区会被刷新. 例如默认情况下, `cin`, `cerr`都关联到`cout`, 因此读`cin`或写`cerr`都会导致`cout`的缓冲区刷新.

##### 刷新输出缓冲区

`flush`刷新缓冲区, 但是不输出任何额外的字符; `ends`向缓冲区插入一个空字符, 然后刷新缓冲区:
...
如果想要在每次输出操作后都刷新缓冲区, 我们可以使用`unibuf`操纵符. 它告诉流在接下来的每次写操作之后都进行一次`flush`操作, 而`nounitbuf`操纵符则重置流, 使其恢复使用正常的系统管理的缓冲区刷新机制
...
当一个输入流和一个输出流关联时, 任何试图从输入流读取数据的操作都会先刷新关联的输出流, 标准库将`cout`和`cin`关联, 因此任何`cin`操作都会导致`cout`的缓冲区被刷新
...
我们既可以将`istream`对象关联到另一个`ostream`, 也可以将`ostream`关联到另一个`ostream`

```c++
cin.tie(&cout); // 表示将cin和cout关联在一块
ostream *old_tie = cin.tie(nullptr); // 表示cin将不再和其它流关联在一起
cin.tie(&cerr); // 键cin和cerr关联到一起
cin.tie(old_tie); // 重建关系
```

每个流同时最多关联到一个流, 但是多个流可以同时关联到同一个`ostream`



### 文件输入输出

`fstream`中定义了三个类型的文件`ifstream`, `ofstream`和`fstream`来支持文件的读写, 常用操作基本和标准IO操作一致.

#### 使用文件流对象

当我们读写文件的时候, 定义一个文件流对象, 并将对象和文件关联起来, 每个文件流类都定义了一个名为`open`的成员函数, 完成一些系统相关的操作, 来定位给定的文件, 并视情况打开为都或者写模式

如果创建文件流对象的时候, 我们提供了文件名称, 那么`open`函数将会自动被调用
...
新版本规定`string`和`cstring`都可以作为文件名参数
...
如果我们定义了空文件流对象, 可以随后用`open`将它和文件关联起来, 如果调用`open`失败, `failbit`会被置位. 因为调用`open`可能失败, 进行`open`检查很有必要:

```c++
if (out) // 检查open是否成功, open成功我们可以打开文件
```

##### 自动构造和析构

当一个`fstream`对象被销毁的时候, `close`会被自动调用
...
无论采用哪种方式打开文件, 我们都可以指定文件模式, 调用`open`时, 但是会有如下限制:

* 只可以对输出流对象设定`out`模式
* 只可以对输入流对象设定`in`模式
* 只有当`out`时才可以设定`trunc`模式
* 只要`trunc`没被设定, 才可以设定`app`模式, 在`app`模式下, 即使没有显式指定`out`模式, 文件也总是以输出方式被打开
* 默认情况下为`trunc`, 以`out`模式打开会被截断, `app`用于追加
* `ate`和`binary`模式可以用于任何类型文件流对象



### `string`流

`sstream`头文件定义了三个类型来支持内存版`IO`, 这些类型向`string`写入数据, 从`string`读取类型.

`istringstream`从`string`读取数据, `ostringstream`向`string`写入数据, `stringstream`可读可写.

```c++
sstream strm; // 没有绑定
sstream strm(s); // 保存s的一个拷贝, 已经绑定
strm.str(); // 返回所保存的拷贝
...
可以理解成相当于把`string`作为了一个缓冲输入.

##### 使用`ostringstream`

首先同样需要绑定一个`string`, 但是由于使用的是副本, 因此需要返回.`str`可以返回和绑定

## 顺序容器

一个容器是一个特定类型对象的集合, 顺序容器为程序员提供了控制元素存储和访问顺序的能力. 但是它们的一些能力会有一些性能折中:
...
`forward_list`和`array`是`C++`新标准增加的类型, `array`是一种更加安全的数组类型
...
所有的迭代器都定义了递增运算符和解引用运算.

##### 迭代器范围

> 迭代器范围是标准库基础

一个迭代器范围由一对迭代器表示, 两个迭代器分别指向同一个容器的元素或者是尾元素之后的位置, 通常称为`begin`和`end`
...
剩下的就是类型别名, 通过类型别名, 我们可以在不了解元素类型的情况下使用它. 如果需要元素类型`value_type`, 需要元素类型的一个引用, 可以使用`reference`或者`const_reference`
...
```c++
swap(c1, c2); // 交换c1和c2中的元素, 拷贝速度更快
seq.assign(); // 赋值运算, 将会导致迭代器失效
```

##### 使用`assign`(仅顺序容器)

赋值运算仅允许左边和右边的运算对象具有相同数据类型, 但是`assign`成员允许进行进行不同但相容的类型赋值, 或者从一个子序列赋值. `assign`操作用参数所指定的元素替换左边容器中的所有元素.
...
操作交换, 操作交换两个类型相同的内容. 调用`swap`两个容器元素将会交换. **实际上内部元素并没有交换, 只是交换了数据结构**

> 在这个过程中不会发生拷贝删除或者插入, 因此指针和迭代其都不会失效

##### 插入范围内元素

除了迭代器第一个参数, `insert`函数可以接受更多的参数, 这与容器构造函数类似

> 在新标准下, 接受元素个数或范围的`insert`版本指向第一个新加入元素的迭代器. 如果范围为空, 不插入任何元素, `insert`操作会将第一个参数返回
...
由于每次调用都会返回当前调用的迭代器, 因此可以不断迭代

##### 使用`emplace`操作

`emplace_front`, `emplace`, `emplace_back`, 这些操作构造而不是拷贝元素, 这些操作对应`push_front`, `insert`和`push_back`, 当调用`push`和`insert`的时候我们是将对象拷贝一份到容器中, 而`emplace`则是传递构造的参数, 直接在容器管理的内存空间中直接构造函数.

> 相当于`emplace`可以直接调用元素类型的构造函数, 从而直接在内存空间中进行构造, 甚至于还可以使用默认构造函数, 也就是直接传入空值
...
与添加元素的多种方式类似, 容器也有多种删除方式

> 删除会导致迭代器失效, 指针失效等

#### 特殊的`forward_list`操作

当添加或删除元素时, 删除或添加的元素之前的那个元素的后继会发生变化. 为了添加或者删除一个元素我们需要访问其前驱. `forward_list`定义了名为`insert_after`, `emplace_after`和`erase_after`的操作
...
我们可以用`resize`来增大或缩小容器, 如果当前大小小于所要求, 后部元素会被删除
...
* `find`返回查找到的下标
  * `find_last_of`找到最后一个
  * `find_last_not_of`搜索最后一个不出现给定`string`中的字符
* 数值转换
  * `to_string`将数值转换为字符串
  * `stod`将字符串转化为浮点数
...
除了顺序容器以外, 标准库还定义了三个顺序容器适配器: `stack`, `queue`和`priority_queue`. 本质上适配器是一种机制, 能使某种事物的行为看起来向另外一种事物一样, `stack`支持顺序容器等等

默认情况下, `stack`和`queue`是基于`dequeue`实现的, `priority_queue`是在`vector`之上实现的

因为适配器要求容器具有增加和删除的能力, 因此`array`无法作为适配器的容器

## 泛型算法

标准库定义了一组**泛型算法**: 例如排序和搜索

大多算法在头文件`algorithm`中, 标准库还在头文件中`numeric`中定义了数值泛型算法

内置数组可以采用`begin`和`algorithm`进行计算头和尾置迭代器.
...
* `accumulate`累加, 第三个参数的类型决定了函数使用哪个加法运算符以及返回值的类型. 

  >  对于只读而不改变元素的算法, 应该通过采用`cbegin`等`const`迭代器

* `equal`, 用于确定两个蓄力是否保存相同的值. 它将第一个序列中的每个元素和序列中的对应元素进行比较.

  > 对于那些只接受单一迭代器来表示第二个序列的算法, 都假定第二个序列至少与第一个序列一样长
...
* `back_inserter`, 插入迭代器保证算法有足够的空间来容纳输出数据, 插入迭代器是一种向容器添加元素的迭代器

  ```c++
  vector<int> vec;
  auto it = back_inserter(vec); // 接受一个指向容器的引用, 返回一个与该容器绑定的插入迭代器, 当我们通过迭代器进行赋值操作的时候, 赋值运算符会紫红`push_back`
  *it = 42;
  
  vector<int> vec;
  fill(back_inserter(vec), 10, 0);
  ```

* `copy`算法

  拷贝算法是向目的位置迭代器指向的输出序列中的元素写入数据的算法. 前两个表示范围, 后一个表示目的, 一般是创建一个新序列, 不是原地修改

#### 重排元素的算法

* `sort`
* `unique`将相邻的重复项进行消除, 其实本质上是进行覆盖相邻的元素, 使得不重复的元素出现在序列的开始, 返回的迭代器指向最后一个不重复元素之后的位置
...
谓词是一个可调用的表达式, 标准库分为: **一元谓词**, **二元谓词**, 分别表示只接受一个参数和两个参数
...
通用的函数适配器, 接受一个可调用对象, 生成一个新的可调用对象来适应原对象的参数列表:

```c++
auto newCallable = bind(callable, arg_list); // 后面是一个参数列表
```

##### 使用`placeholders`名字

名字`_n`都定义在一个名为`placeholders`的命名空间, 这个命名空间本省定义在`std`命名空间中, 因此使用前必须要进行声明
...
* 插入迭代器
  ...
  * `back_inserter`, 解引用赋值相当于`push_back`
  * `front_inserter`, 同理
  * `inserter`, 同理
...
* 反向迭代器
  ...
  反向迭代器就是在容器中从尾元素向首元素反向移动的迭代器, 因此需要一个`--`操作
...
`[]`如果不存在会自动插入并进行初始化, `at`则会抛出异常

## 内存管理

### 动态内存和只能指针

`C++`采用`new`和`delete`来动态的分配释放内存空间, 为了更加安全地使用, 新标准提供了智能指针:

* `shared_ptr`允许多个指针指向同一个对象
* `unique_ptr`独占所指对象
* `weak_ptr`伴随类, 弱引用, 指向`shared_ptr`所管理地对象
...
```c++
shared_ptr<string> p1; // 默认初始化的智能指针中保存着一个空指针
if (p1 && p1->empty())
    *pi = "hi";
```

| `shrared_ptr<T> sp`, `unique_ptr<T> up` | 空智能指针, 可以指向类型为`T`的对象                          |
| --------------------------------------- | ------------------------------------------------------------ |
| `p`                                     | 是否指向对象                                                 |
| `*p`                                    | 解引用                                                       |
| `p->mem`                                | 获取成员                                                     |
| `p.get`                                 | 返回其中的保存指针                                           |
| `swap`                                  | 交换`p`和`q`中的指针                                         |
| `make_shared<T>(args)`                  | 返回一个`shared_ptr`, 指向动态分配的类型`T`对象              |
| `shared_ptr<T>p (q)`                    | `p`是`shared_ptr`的拷贝, 此操作会递增`q`中的计数器. `p`中的指针必须能转换为`T*` |
| `p = q`                                 | `p`和`q`都是`shared_ptr`, 所保存的指针必须能相互转换, 此操作会递减`p`的引用计数, 递增`q`的引用计数, 若`p`的引用计数变为0, 则将其管理的原内存释放 |

##### `make_shared`函数

```c++
auto p3 = make_shared<int>(42); // 类似顺序容器的`emplace`
```

##### `shared_ptr`的拷贝和赋值

每个`shared_ptr`都有一个关联的计数器, 通常称其为**引用计数**, 无论何时拷贝`shared_ptr`, 计数器都会增加. 进行常规的含有拷贝操作的行为都会导致引用计数的增加, 而**赋值一个新值或者`shared_ptr`被销毁**时, 计数器就会递减, 一旦一个计数器变为0, 它就会自动释放自己所管理的对象.

```c++
auto r = make_shared<int>(42);
r = q; // r指向int的一个引用, 给r赋值, r就会递减, q会增加引用计数, 从而r之前的计数会被释放
```
...
```c++
int *p1 = new int;
int *p2 = new (nothrow) int; // 如果分配失败, new返回空指针, 否则就会抛出异常
```

#### `shared_ptr`和`new`结合使用

`shared_ptr`必须使用显式的初始化对`new`进行初始化, 不存在隐式转化

#### `unique_ptr`

拥有, 不支持拷贝和赋值操作, 直接初始化

* `release`放弃控制权, 返回指针, 并将`u`置空

> 不能拷贝`up`的一个例外是我们可以拷贝或赋值一个将要被销毁的`unique_ptr`, 比如从函数返回一个`unique_ptr`
...
发布新标准之前, 类是通过将拷贝构造函数和拷贝赋值运算符声明为`private`来阻止拷贝.
...
* 每个对象还要创建一个引用计数, 用来记录有多少个对象与正在创建的对象共享状态
* 拷贝构造函数不分配新的计数器, 而是拷贝给对象的数据成员, 递增
* 析构使计数器减一, 为0时释放
* 拷贝赋值运算的时候, 左减右加

**解决问题之道在于将计数器保存在动态内存当中, 就是都指向同一片内存空间, 某一种对象共享同一片内存空间**
...
除了定义拷贝控制成员之外, 通常管理资源的类还定义一个名为`swap`的函数, 也就是只交换内部结构, 而实质上数据并没有发生移动.
...
> 对于资源管理的类来说, 交换操作可以进行大幅度的优化, 这个是我们自行定义的swap, 而非标准库中的swap
...
> 交换时我们采用`using std::swap`然后使用无限定`swap`

##### 在重新分配内存的过程中移动而不是拷贝元素

##### 移动构造函数和`std::move`

移动构造函数通常是将资源从给定的对象"移动"而不是拷贝到正在创建的对象.

第二个机制是名为`move`的标准库函数, 调用表示希望使用移动构造函数

### 对象移动

对象拷贝后立即销毁是很耗时间的, 而移动可以大幅提高效率

#### 右值引用

新标准引入新的引用类型**右值引用**, 也就是必须要绑定到右值的引用, 我们通**&&**来来获得右值引用, 它只能绑定到一个将要销毁的对象, 因此我们可以自由的将一个右值引用的资源移动到另一个对象中
...
变量表达式也有左值和右值属性, 变量表达式都是左值, 带来的结果就是我们不能将一个右值引用绑定到一个右值引用类型的变量上:
...
虽然不能将一个右值引用直接绑定到一个左值上, 但是我们可以显示的将一个左值转化为对应的右值引用类型, 我们还可以通过调用一个名为`move`的新标准函数来获得绑定到左值上的右值引用.

```c++
int &&rr3 = std::move(rr1); // ok
```

`move`调用高数编译器我们有一个左值, 但是我们希望像右值一样处理它, 我们必须认识到, 调用`move`意味着, 除了对`rr1`赋值或者销毁它外, 我们将不再使用它. 在`move`之后, 我们不能对移后源对象的值做任何假设.

> 我们可以销毁一个移后源对象, 也可以赋予它新值, 但不能使用一个移后源对象的值

> 使用move的代码应该使用`std::move`而不是`move`, 防止潜在名字冲突

#### 移动构造函数和移动赋值运算符

为了让自己的类型支持移动操作, 需要定义移动构造函数和移动赋值运算符, 可以从对象**窃取**资源而不是**拷贝资源**.

移动构造函数的第一个参数是该类类型的一个引用, 但是是右值引用. 完成了资源移动, 移动构造函数还必须确保移后源对象处于这样一个状态--销毁它是无害的. 一旦资源完成移动, 源对象必须不再指向被移动的资源, 这些资源的所有权已经归属新创建的对象.
...
由于移动操作的窃取资源, 因此不会抛出任何异常, 因此可以通知标准库`noexcept`, 否则它会认为移动我们的类对象时可能抛出异常

> 不抛出任何异常的移动构造函数和移动赋值运算符必须标记为`noexcept`
...
如果类定义了自己的拷贝构造函数, 拷贝赋值运算符或者析构函数, 编译器就不会为它合成移动构造函数和移动赋值运算符

只有当一个类没有定义自己任何版本的拷贝控制成员, 且类的每个非`static`数据成员都可以移动时, 编译器才会为它合成移动控制的函数. 编译器可以移动内置类型的成员, 如果成员是一个类类型, 但是可以移动, 那么编译器也可以移动该成员.



> 只有当一个类没有定义任何自己版本的拷贝构造控制成员, 且它的所有数据成员都能有移动构造或移动赋值时, 编译器才会为它合成移动构造函数或移动赋值运算符

##### 移动右值, 拷贝左值...

移动构造函数接受一个`&&`右值引用, 只能用于实参是右值的情况

```c++
StrVec v1, v2;
v1 = v2; // v2是左值, 使用拷贝构造
StrVec getVec(istream &); // 返回一个右值
v2 = getVec(cin); // 是一个右值, 使用移动赋值
```

我们无法隐式地将一个右值引用绑定到一个左值

##### 如果没有移动构造函数, 右值也被拷贝

由于`&&`可以被`const &`所捕捉, 因此拷贝函数代替移动构造函数几乎肯定是安全地.

> 如果一个类有一个可用地拷贝构造函数而没有移动构造函数, 则其对象是通过拷贝构造函数来移动的
...
> 三五法则:
>
> 一般来说, 如果一个类定义了任何一个拷贝操作, 它就应该定义所有的5个操作, 移动构造和赋值一般是为了减小系统开支

> 不要随意使用移动操作, 由于一个对象源移动后具有不确定的状态, 对其调用`std::move`是非常危险, 当我们调用`move`时, 必须绝对要确认移后源对象没有其它用户
...
我们指出`this`的左值/右值属性的方式与`const`成员函数相同, 在参数列表之后放置引用限定符

```c++
Foo &Foo::operator=(const Foo &rhs) & {
    return *this;
} // 引用限定符可以是&和&&, 分别指出this可以指向一个左值或者右值, 只能用于成员函数
// 返回&左值可以修改, &&右值不可修改
```

> 如果`const`限定符也存在, 那么`const`只能在前, 另一个在后.
...
d.Base::memfcn(); // 正确, 调用了Base::memfcn
```

##### 虚函数和作用域

基类和派生类的虚函数必须采用相同的形参列表, 假如不同, 则无法通过基类的指针或者引用调用派生类的虚函数了.

##### 通过基类调用隐藏的虚函数

> 调用时采用的是静态调用, 也就是我们的指针类型决定了我们能调用哪一种函数, 但是如果调用的是虚函数, 那么只有在运行的时候编译器才会决定这个函数具体绑定哪个函数.

##### 覆盖重载的函数

成员函数无论是否是虚函数都能重载. 派生类可以覆盖重载函数的0个或者多个实例. 如果派生类希望所有的重载版本对它来说都是可见的, 那么它就需要覆盖所有的版本, 或者一个也不覆盖.

有时一个类仅需要覆盖重载集合中的一些而非全部函数, 此时, 如果我们不得不覆盖基类中的每一个版本的话, 显然操作操作将极其繁琐.

我们可以为重载的成员提供一条`using`声明语句, 这样就无须覆盖基类中的每一个重载版本了, `using`声明语句指定一个名字而不指定形参列表, 所以一条基类成员函数的`using`声明语句就可以把该函数的重载实例添加到派生类的作用域中. 此时派生类只需要定义其特有的函数就可以了, 而无须为继承而来的其它函数重新定义

类内`using`声明的一般规则同样适用于重载函数的名字, 基类函数的每一个实例在派生类都必须是可访问的, 对派生类没有重新定义的重载版本的访问, 实际上是对`using`声明点的访问.
...
继承关系对基类拷贝最直接的影响是基类通常应该定义一个虚析构函数, 这样我们就能直接动态分配继承体系中的对象
...
函数模板会自动推断, 我们可以隐式的指定默认实参

除了定义类型参数, 还可以在模板中定义非类型参数, 表示一个值而非一个类型
